...
public:
    ...
    std::vector<OldChunkSnapshot> chunks;
};

template<typename S>
void serialize(S& serializer, OldTileMapSnapshot& testTileMap)
{
    ...
    serializer.container(testTileMap.chunks,
                         static_cast<std::size_t>(OldTileMapSnapshot::MAX_CHUNKS));
}
